...
def findMaxContour(contours): #max contours bulmak
    ...
    for i in range(len(contours)): #alanları tek tek birbirleriyle karşılaştırarak max olana ulaşmak
        ...
        if max_area<area_hand: #burda max alanı bulana kadar döndürür.
            max_area=area_hand
            max_i=i
...
while True:
    ...
    cv2.rectangle(frame,(200,100),(350,300),(0,0,255),0) #(x1,y1),(x2,y2).kalınlık sıfır olsun ki o da mask işlemine dahil olmasın
    ...
    if len(contours) > 0: #contours dan herhangibir değer gelmediyse
        ...
        try: #arka plandaki beyaz karıncalanmalaı ilgilnemke istemiyorum sadece yüzümle ilgilenmek istiyorum
            ...
            cv2.line(roi, extrRight, extLeft, (255, 0, 0), 2)


            a=math.sqrt((extrRight[0]-extrTop[0])**2+(extrRight[1]-extrTop[1])**2) #kenarının uzunluğu
            c = math.sqrt((extrTop[0] - extrRight[0]) ** 2 + (extrTop[1] - extrRight[1]) ** 2)#buralarda iki nokta rasındaki uzaklık formülünü uyguluyoruz.
            b = math.sqrt((extrRight[0] - extLeft[0]) ** 2 + (extrRight[1] - extLeft[1]) ** 2)
